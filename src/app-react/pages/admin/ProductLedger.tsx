...
import { Product, Order, getProducts, getOrders, updateProduct, addProduct, deleteProduct, ScheduledPrice, getScheduledPrices, addScheduledPrice, deleteScheduledPrice, applyScheduledPrices, getGlobalProducts, getGlobalOrders, getBranches, getCurrentBranch, getCategories, addCategory, deleteCategory } from '../../utils/mockData';
...
export default function ProductLedger() {
  ...
  // Initialize selectedBranch & branches
  useEffect(() => {
    const initBranches = async () => {
      try {
        if (isSuperAdmin) {
          ...
          setBranches(list);
          setSelectedBranch(list[0] || 'Palembang');
        } else {
          setSelectedBranch(user?.branch || 'Palembang');
        }
      } catch (err: any) {
        toast.error('Gagal menginisialisasi cabang: ' + err.message);
      }
    };
    initBranches();
  }, [isSuperAdmin, user?.branch]);

  // Load products, orders, categories and dynamic prices
  const loadData = useCallback(async () => {
    if (!selectedBranch) return;
    setIsLoading(true);
    try {
      // Proactively trigger scheduled price executions first
      await applyScheduledPrices();

      const [productsData, ordersData, pricesData, categoriesData] = await Promise.all([
      ...
      const filteredProducts = productsData.filter(p => (p as any).branch === selectedBranch);
      const filteredOrders = ordersData.filter(o => (o as any).branch === selectedBranch);

      setProducts(filteredProducts);
      setOrders(filteredOrders);
      setScheduledPrices(pricesData);
      setCategories(categoriesData);

      if (!newProductCategory && categoriesData.length > 0) {
        setNewProductCategory(categoriesData[0]);
      }
    } catch (err: any) {
      toast.error('Gagal memuat produk & kategori: ' + err.message);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleAddCategory = async () => {
    ...
    try {
      ...
      toast.error('Gagal menambahkan kategori: ' + e.message);
    }
  };
  ...
  const handleAddScheduledPrice = async () => {
    ...
    try {
      ...
      toast.success('Jadwal perubahan harga berhasil ditambahkan');
      setIsSchedulingPrice(false);
      setNewSchedPrice('');
      setNewSchedDate('');
      loadData();
    } catch (err: any) {
      toast.error('Gagal menjadwalkan harga: ' + err.message);
    }
  };
  ...
  const handleRemoveScheduledPrice = async (id: string) => {
    try {
      ...
      toast.success('Jadwal perubahan harga dihapus');
      loadData();
    } catch (err: any) {
      toast.error('Gagal menghapus jadwal harga: ' + err.message);
    }
  };
  ...
  const handleDeleteProduct = async (id: string) => {
    if (confirm('Apakah Anda yakin ingin menghapus produk ini? Semua stok & riwayat penjualan produk ini akan tetap tersimpan di database.')) {
      try {
                ...
                >
                  ...
                  <div className="flex-1 overflow-hidden">
                    <div className="flex items-center justify-between">
                      <h3 className={`font-semibold truncate text-sm ${selectedProductId === product.id ? 'text-blue-700' : 'text-gray-800'}`}>
                        {product.name}
                      </h3>
                      {scheduledPrices.some(sp => sp.productId === product.id) && (
                        <Calendar className="w-3.5 h-3.5 text-orange-500 animate-pulse" />
                      )}
                    </div>
                  ...
                  {productTransactions.length > 0 ? (
                    <div className="space-y-4">
                      {productTransactions.map((order) => {
                        ...
                        return (
                          <div key={order.id} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow bg-white animate-in fade-in">
